...
const KycPage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await updateUserKycStatus(userId, 'pending', downloadURL);
  ...
  return (
    <div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        {auth.currentUser && (
            <div className="mt-6">
                <p>Current Status: <span className="font-semibold">{auth.currentUser.kycStatus || 'Not Submitted'}</span></p>
            </div>
        )}
      </div>
    </div>
