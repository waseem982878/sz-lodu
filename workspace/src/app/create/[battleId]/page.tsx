...
export default function CreateBattlePage({ params }: { params: { battleId: string } }) {
  ...
  useEffect(() => {
    if (!battleId || !user) {
        // router.push('/play'); // This can cause loops if auth is still loading
        return;
    };
    const unsubscribe = getBattle(battleId, (battleData) => {
        if (battleData) {
            if (battleData.creator.id !== user.uid) {
                 // Opponent has joined, redirect them to the game page.
  ...
  const isPlayerReady = user && battle.readyPlayers && battle.readyPlayers[user.uid];
   ...
   const renderGameControl = () => {
        if (!isOpponentJoined) {
            return (
                <>
                    <p className="text-lg font-semibold my-4">Waiting for opponent to join...</p>
        ...
        if (battle.status === 'inprogress') {
            return (
                <>
                    ...
                    <LudoLaunchButton roomCode={battle.roomCode} />
                </>
            )
        }
        
         return <p className="text-center text-muted-foreground">Game is starting...</p>;
    }
        ...
        <CardContent className="p-4">
          <div className="flex justify-between items-center text-center mb-4">
              <div className="flex flex-col items-center gap-1">
                  <Image src={userProfile.avatarUrl} alt="You" width={40} height={40} className="rounded-full ring-2 ring-primary" data-ai-hint="user avatar" />
